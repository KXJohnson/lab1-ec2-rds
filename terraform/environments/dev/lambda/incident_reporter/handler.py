...
def write_incident_objects_to_s3(incident_id, evidence, report_markdown):
    ...
    final_markdown = report_markdown + build_required_query_appendix(evidence)
...
def build_required_query_appendix(evidence):
    app = evidence.get("logs_insights", {}).get("app", {})
    waf = evidence.get("logs_insights", {}).get("waf", {})

    required_queries = [
        (
            "App error rate over time using bin(1m)",
            "evidence.logs_insights.app.app_error_rate_over_time",
            app.get("app_error_rate_over_time", {}),
        ),
        (
            "App latest 50 DB-related error lines",
            "evidence.logs_insights.app.app_latest_50_db_error_lines",
            app.get("app_latest_50_db_error_lines", {}),
        ),
        (
            "WAF allow vs block",
            "evidence.logs_insights.waf.waf_allow_vs_block",
            waf.get("waf_allow_vs_block", {}),
        ),
        (
            "WAF top blocked IP/URI pairs",
            "evidence.logs_insights.waf.waf_top_blocked_ip_uri_pairs",
            waf.get("waf_top_blocked_ip_uri_pairs", {}),
        ),
    ]
    ...
    lines = [
        "",
        "",
        "## Required Query Evidence Summary",
        "",
        "| Requirement | Evidence Key | Status | Result Rows |",
        "|---|---|---:|---:|",
    ]

    for requirement, evidence_key, query_result in required_queries:
        status = query_result.get("status", "Unknown")
        row_count = len(query_result.get("results", []))
        lines.append(f"| {requirement} | `{evidence_key}` | {status} | {row_count} |")

    lines.extend(
        [
            "",
            "This deterministic appendix is generated by the Lambda from evidence JSON to prove that all required Bonus H query keys were collected.",
            "",
        ]
    )
